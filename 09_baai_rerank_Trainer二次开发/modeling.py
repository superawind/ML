...
class CrossEncoder(nn.Module):
    ...
    def gradient_checkpoint_enable(self, **kwargs):
        self.hf_model.gradient_checkpointing_enable(**kwargs)

    def forward(self, batch):
        # batch.input_ids.shape == [per_device_train_batch_size * group_size, max_len]
        ranker_out: SequenceClassifierOutput = self.hf_model(**batch, return_dict=True)
        logits = ranker_out.logits # ModelBert输出池化之后, [bs, seq_len=max_len, hidden_size] -> [bs, hidden_size] 经过一个 ffn [hidden_size, num_labels=1] 此处等于1 处理后的值
